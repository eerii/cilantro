...
def eliminacion_gauss(sistema, columna, salida=True):
    ...
    for f in eliminacion_gauss.outras_filas:
        ...
        if salida:
            print("Fila: {}, a_{}{} / a_{}{} = {:.2f}".format(f+1, f+1, columna+1, eliminacion_gauss.fila_pivote+1, columna+1, k))
        ...
        if salida:
            stringify = lambda arr : " ".join([str(round(x,2)) for x in arr[:-1]]) + " | " + str(round(arr[-1], 2))
    ...
    if salida:
        print_matriz(np.round(sistema, 2), "\nSistema:", "")

# Triangulación con pivoteo parcial:
# Transforma unha matriz nunha matriz triangular superior utilizando o método de gauss descrito anteriormente
# Ademáis, aplica o algoritmo de pivoteo parcial para colocar as filas na posición ótima
def triangulacion_con_pivoteo_parcial(ecuacions, fila_pivote=0, salida=True):
    sistema = ecuacions.astype(float) # Sistema de ecuacions sobre o que operar
    n_columnas = len(sistema[0]) # Número de columnas
    columna = lambda a, n: a[:, n] # Obter a columna dun array 2D
    ...
    for i in range(n_columnas - 1):
        ...
        if fila_max != i:
            ...
            print("Pivoteo: Fila {} a {}".format(fila_max+1, i+1))
            print_matriz(np.round(sistema, 2), "Novo sistema:", "")
...
# Triangulamos o sistema polo método de gauss con pivote
sistema_triangular = triangulacion_con_pivoteo_parcial(ec)
